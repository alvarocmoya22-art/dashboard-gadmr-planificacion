...
export function Processes() {
  ...
  return <div>
    <div className="page-actions"><div className="view-tabs"><button className="active">Todos los procesos</button><button><SlidersHorizontal size={15} /> Mis filtros</button></div><div>{canExportReports(role) && <Button variant="secondary" onClick={() => exportProcessesToXlsx(processes)}><Download size={17} /> Exportar</Button>}{canCreateProcesses(role, userEmail) && <Button onClick={() => setFormProcess(null)}><Plus size={17} /> Nuevo proceso</Button>}</div></div>
