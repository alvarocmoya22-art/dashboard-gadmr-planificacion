...
export function ProcessDetail() {
  ...
  async function addRelated() {
    const num = newRelated.replace(/\D/g, '')
    if (!num) return
    try { await addManualRelated(currentProcess.id, num); setNewRelated('') }
  ...
  return <div className="detail-page">
    ...
    <div className="detail-grid"><div className="detail-main">
      ...
      {(relatedIssues.length > 0 || manualRelated.length > 0 || canEdit) && <Card className="info-card"><h3><ExternalLink size={18} /> Trámites relacionados</h3>
        {relatedIssues.length > 0 ? <div className="madre-list">{relatedIssues.map((num) => {
          const d = madreDetalle.find((x) => String(x.issue) === num)
          return <div className="madre-item" key={num}>
            <div className="madre-head"><span className="madre-tag">Madre</span><a href={`${EGOB_BASE_URL}/issues/${num}`} target="_blank" rel="noreferrer">eGob #{num} <ExternalLink size={12} /></a></div>
...
function OperativeEdit({ process }: { process: Process }) {
  const { statuses, saveProcess, markReviewed } = useApp()
  ...
  return <Card className="operative-edit">
    ...
    <div className="operative-edit-grid">
      ...
      <div className="operative-edit-review">
        <button type="button" className="operativa-link" onClick={() => void markReviewed(process.id)} disabled={saving} title="Lo oculta de Pendientes por hoy (no borra la fecha)"><CheckCircle2 size={15} /> Marcar como revisado</button>
      </div>
    </div>
