...
export function ProcessDetail() {
  ...
  return <div className="detail-page">
    ...
    <div className="detail-grid"><div className="detail-main">
      ...
      <Card className="info-card"><h3><ExternalLink size={18} /> Vinculación eGob / eDoc</h3>{egobUrl ? <div className="egob-card"><div><span>Nro. trámite eGob</span><strong>{egobNumber}</strong></div><div><span>Estado eGob</span><strong>{process.egob_estado || 'Pendiente de sincronizar'}</strong></div><div><span>Último movimiento</span><strong>{process.egob_ultimo_movimiento || 'Pendiente de sincronizar'}</strong></div><a className="button button-secondary" href={egobUrl} target="_blank" rel="noreferrer"><ExternalLink size={16} /> Abrir en eGob</a></div> : <p className="all-clear">Este trámite aún no tiene número eGob/eDoc asociado.</p>}</Card>
