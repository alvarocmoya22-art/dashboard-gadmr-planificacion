...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  return <div className="auth-screen">
    ...
    <form className="auth-card" onSubmit={login}>
      ...
      <Field label="Correo electrónico"><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></Field>
      <Field label="Contraseña"><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></Field>
      <Button type="submit">Iniciar sesión</Button>
      <small>El acceso y las acciones se registran según tu rol.</small>
    </form>
  </div>
}
